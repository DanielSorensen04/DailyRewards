package me.dalot.dailyrewards.database;
...
public class MySQLManager {
    ...
    public static void createPlayer(final String uuid) {
        ...
        try (Connection connection = getConnection()) {
            ...
            DailyRewardsPlugin.get().getLogger().warning("Fejl ved oprettelse af spilleren " + uuid + "!\n" + ex.getLocalizedMessage());
        }
    }
    ...
    public static boolean playerExists(final String uuid) {
        try (Connection connection = getConnection()) {
            ...
            DailyRewardsPlugin.get().getLogger().warning("Fejl ved tjek om spilleren " + uuid + " eksisterer!\n" + ex.getLocalizedMessage());
            return false;
        }
    }
    ...
    public static void updateCooldown(final UUID uuid, Map<String, Object> data) throws SQLException {
        ...
        try (Connection connection = getConnection()) {
            ...
            DailyRewardsPlugin.get().getLogger().warning("Fejl ved opdatering af cooldown for " + uuid + "!\n" + ex.getLocalizedMessage());
        }
    }
    ...
    private static Connection getConnection() {
        if (dataSource == null) {
            throw new SQLException("Kunne ikke tilslutte fordi data source er null.");
        }
        Connection connection = dataSource.getConnection();
        if (connection == null) {
            throw new SQLException("Kunne ikke tilslutte.");
        }
        return connection;
    }
}
